...
class MatchCat:
    def __init__(self, img_name :str):
        ...
        self.match_dir = match_base + img_name +'/'
        img_dir = image_base + img_name +'/'
        if not os.path.exists(img_dir):
            exit(f'Image directory {img_dir} does not exist')
        ...
        if not os.path.exists(self.tab_path):
            exit(f'Matched catalog {self.tab_path} does not exist')
    ...
    def interactive_match(self, hst_path :str):
        ...
        while i <len(tab_ref_matched):
            ...
            fig.suptitle(f'EGS-{tab_ref_matched["Seq"][i]}')
    ...
    def get_image(self, hst_path: str, i: int, img_size: int = 50):
        ...
        print(f'Object {i+1} of {len(tab_ref_matched)}: EGS-{tab_ref_matched["Seq"][i]}')
